...
/**
 * Parse hook JSON from stdin. Strips UTF-8 BOM (U+FEFF) and leading junk
 * so BOM-prefixed or wrapped payloads from Cursor still parse.
 * @param {string} raw
 * @returns {object}
 */
export function parseHookStdinJson(raw) {
  let s = typeof raw === "string" ? raw : String(raw);
  // Strip BOM / zero-width no-break space that some editors or pipes add
...
export async function readStdin() {
  return new Promise((resolve, reject) => {
    ...
    process.stdin.on("end", () => {
      try {
        resolve(parseHookStdinJson(data));
      } catch (e) {
...
export function generateSessionId(workspaceRoots) {
  ...
  const folderName = basename(root.replace(/\\/g, "/")) || root;
